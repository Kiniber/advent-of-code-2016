...
pub fn part1(input: &str) -> anyhow::Result<()> {
    let regex = Regex::new(r"(?<encrypted_name_part>(([a-z]+)-?)+)-(?<sector_id>\d+)\[(?<checksum>[a-z]+)\]")?;
    let results = regex.captures_iter(input).collect::<Vec<_>>();
    let real_rooms_amount = results.into_iter().map(|captures|{
...
impl <'a> Room<'a> {
    ...
    fn validate(&self) -> bool {
        ...
        if amount == 5 {
            let details = set.iter()
                //.take(10)
                .fold(String::new(),|mut builder, SortingElement{amount, chr}|{
    ...
    fn count_chars(encrypted_name_part: &str) -> BTreeSet<SortingElement> {
        ...
        map.into_iter().map(|(chr, amount)|SortingElement{amount, chr})
