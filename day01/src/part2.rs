...
fn try_parse_path_fragment(input: &str) -> anyhow::Result<PathFragment> {
    ...
    match direction {
        ...
        unknown_direction => Err(Day1ApplicationError::UnknownDirectionError {
            unknown_direction: unknown_direction.to_string(),
        }
        .into()),
    }
}
...
fn append_next_path_fragment(
    global_position: &mut (i16, i16),
    current_direction: &mut (i16, i16),
    fragment_to_append: &PathFragment,
) {
    ...
    match current_direction {
        ...
        (x, y) => {
            let err = Day1ApplicationError::UnknownMovingDirection {
                x: x.clone(),
                y: y.clone(),
            };
            // TODO: throw the Error instead of just printing it
            println!("{err}");
        }
    }
}
